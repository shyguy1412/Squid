...
export function %%NAME%%(options?:RequestInit) {
  return fetch('%%PATH%%', options);
}
`;

const moduleTemplate = `export function %%NAME%%(options: RequestInit): Promise<Response>;`;
    ...
    pluginBuild.onStart(async () => {
      ...
      for (const entryPoint of pluginBuild.initialOptions.entryPoints as string[]) {
        ...
        const exportStatement = exportLambdaTemplate
          ...
          .replace('%%PATH%%', lambdaPath.replace(/^(http(s)?:\/\/)?/, 'http$2://'));
      ...
      await fs.writeFile('./build/lambda.ts', exportStatements.join('\n'));
...
export const LambdaApiPlugin = ({
  ...
  setup(pluginBuild) {
    ...
    pluginBuild.onResolve({ filter: /squid-ssr\/lambda/ }, async () => {
      ...
      return {
        path: path.resolve('./build/lambda.ts')
      };
    });
  }
} as Plugin);